...
interface PinItemCardProps {
  ...
  onImageClick?: (pin: PinData) => void; // New prop for image click
}

export function PinItemCard({
  pin,
  onAddAudio,
  onDeletePin,
  onImageClick,
      ...
      <CardHeader className="p-0 relative">
        {imageLoading && (
          <div
            ...
            style={{
              aspectRatio:
                pin.width && pin.height ? `${pin.width}/${pin.height}` : "4/3",
            }}
          ></div>
        )}
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <CardDescription className="text-xs text-muted-foreground truncate cursor-help">
                ...
                Model: {pin.modelUsed || "default"}, Seed:{" "}
                {pin.seed || "random"}
              </CardDescription>
            </TooltipTrigger>
            <TooltipContent>
              <p>Original Prompt: {pin.originalPrompt}</p>
              {pin.originalPrompt !== pin.finalPrompt && (
                <p>Enhanced Prompt: {pin.finalPrompt}</p>
              )}
            </TooltipContent>
          </Tooltip>
        </TooltipProvider>

        <CardTitle className="mt-1 text-sm font-normal line-clamp-3">
          {pin.finalPrompt}
        </CardTitle>
